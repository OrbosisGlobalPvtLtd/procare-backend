...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.category.edit', ['category' => $category->id, 'lang_code' => admin_lang()])->with($notification);
    }
    ...
    public function update(Request $request,$id)
    {
        ...
        if ($request->lang_code !== 'en') {
            updateOrCreateTransaltion($request, $category);
        }
        ...
        if($request->icon && ($request->lang_code === 'en')){
        ...
        if($request->image && ($request->lang_code === 'en')){
    ...
    public function destroy($id)
    {
        ...
        if($count == 0){
            ...
            $category->deleteAllTranslations();
            $category->delete();
            if($old_logo){
                if(File::exists(public_path().'/'.$old_logo))unlink(public_path().'/'.$old_logo);
            }
