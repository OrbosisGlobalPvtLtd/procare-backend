...
class NearestLocationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.nearest-location.edit',['nearest_location' => $location->id, 'lang_code' => admin_lang()])->with($notification);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->lang_code !== 'en'){
            updateOrCreateTransaltion($request, $location);
        }else{
            $location->location = $request->location;
            $location->status = $request->status;
            $location->save();
    ...
    public function destroy($id)
    {
        ...
        if($count == 0){
            $location = NearestLocation::find($id);
            $location->deleteAllTranslations();
            $location->delete();
