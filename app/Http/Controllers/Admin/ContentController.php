...
class ContentController extends Controller
{
    ...
    public function updateMobileApp(Request $request){
        $rules = [
            'app_full_title'=>'required',
            'app_description'=>'required',
        ];

        if($request->lang_code == 'en'){
            $rules['play_store'] = 'required';
            $rules['app_store'] = 'required';
        }

        $customMessages = [
            'app_full_title.required' => trans('admin_validation.Title is required'),
        ...
        if($request->lang_code !== 'en'){
            updateOrCreateTransaltion($request, $setting, 'settings');
        }else{
            $setting->app_full_title = $request->app_full_title;
    ...
    public function defaultPlaceholder(){
        ...
        $default_placeholder = array(
            'image' => $setting->default_placeholder
        );
        $default_placeholder = (object) $default_placeholder;
    ...
    public function updateDefaultPlaceholder(Request $request){
        ...
        if($request->placeholder){
            $existing_placeholder = $setting->default_placeholder;
            $extention = $request->placeholder->getClientOriginalExtension();
            $default_placeholder = 'default-placeholder'.date('-Y-m-d-h-i-s-').rand(999,9999).'.'.$extention;
            $default_placeholder = 'uploads/website-images/'.$default_placeholder;
            Image::make($request->placeholder)
                ->save(public_path().'/'.$default_placeholder);
            $setting->default_placeholder = $default_placeholder;
            $setting->save();
            if($existing_placeholder){
                if(File::exists(public_path().'/'.$existing_placeholder))unlink(public_path().'/'.$existing_placeholder);
            }
        }
    ...
    public function update_homepage(Request $request){
        ...
        if($request->lang_code !== 'en'){
            updateOrCreateTransaltion($request, $homepage, 'homepages');
        }else{
