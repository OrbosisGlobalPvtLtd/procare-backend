...
public function updateApprovalStatus(Request $request, $id)
{
    ...
    public function showRegisterForm()
    {
        ...
        return view('builder.register', compact('countries'));
    }
    ...
    public function property_city_list(Request $request, $id)
    {
        ...
        return response()->json([
            'template' => view('city_partials', compact('cities', 'city_id'))->render()
        ], 200);
    }

    public function property_city_list_filtering(Request $request, $id)
    {
        $cities = City::where('country_id', $id)->orderBY('name', 'asc')->get();
        $city_id = $request->city_id ?? null;

        return response()->json([
            'template' => view('city_partials2', compact('cities', 'city_id'))->render()
        ], 200);
    }
    ...
    public function agencies(Request $request)
    {
        ...
        $sub_agents = User::whereIn('owner_id', $agent_arr)->where('status', 1)->get();
    ...
    public function agency_details(Request $request, $id)
    {
        ...
        return view('agency_details')->with([
            'agency' => $agency,
            'agents' => $agents,
            'properties' => $properties,
            'mobile_app' => $mobile_app,
    ...
    public function language_switcher(Request $request)
    {
        ...
        // Fetch language direction based on the requested language code
        $request_lang = Language::where('lang_code', $request->lang_code)
            ->pluck('lang_direction')
            ->first();

        // Fetch default language direction if no specific language is requested
        $default_lang = Language::where('is_default', 'yes')
            ->pluck('lang_direction')
            ->first();

        // Set defaults
        $lang_direction = $request_lang ?? $default_lang ?? 'left_to_right';
        $lang_code = $request->lang_code ?? config('app.locale');

        // Override direction for Arabic
        if ($lang_code === 'ar') {
            $lang_direction = 'right_to_left';
        }

        // Store the language settings in the session
        Session::put('lang_dir', $lang_direction);
        ...
        $notification = trans('user_validation.Language switched successfully');
        $notification = [
    ...
    public function admin_language_switcher(Request $request)
    {
        // Fetch language direction based on the requested language code
        $request_lang = Language::where('lang_code', $request->lang_code)
            ->pluck('lang_direction')
        ...
        // Store the language settings in the session
        Session::put('lang_dir', $lang_direction);
        Session::put('admin_lang', $lang_code);
    ...
    public function currency_switcher(Request $request)
    {
        ...
        $request_currency = MultiCurrency::where('currency_code', $request->currency_code)->firstOrFail();
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if (isset($rates[$request->currency_code])) {
                    $current_rate = $rates[$request->currency_code];
                    ...
                    $notification = trans('user_validation.Currency switched successfully');
                    $notification = array('messege' => $notification, 'alert-type' => 'success');
                    return redirect()->back()->with($notification);
                } else {
                    $notification = trans('user_validation.Fetch faild to server');
                    $notification = array('messege' => $notification, 'alert-type' => 'error');
                    return redirect()->back()->with($notification);
                }
            } else {
    ...
    /**
     * Return states by country (JSON) for registration location dropdowns.
     */
    public function statesByCountry($country_id)
    {
        try {
            $states = \Illuminate\Support\Facades\DB::table('country_states')
                ->where('country_id', $country_id)
                ->orderBy('name')
                ->get(['id', 'name', 'code']);
            return response()->json(['states' => $states], 200);
        } catch (\Exception $e) {
            \Log::error('statesByCountry: ' . $e->getMessage());
            return response()->json(['states' => [], 'error' => 'Failed to load states'], 200);
        }
    }

    /**
     * Return cities by state (JSON) for registration location dropdowns.
     * Supports both country_state_id and state_id columns in cities table.
     */
    public function citiesByState($state_id)
    {
        try {
            ...
            $stateColumn = \Illuminate\Support\Facades\Schema::hasColumn($table, 'country_state_id')
                ? 'country_state_id'
                : 'state_id';
            $cities = \Illuminate\Support\Facades\DB::table($table)
                ->where($stateColumn, $state_id)
                ->orderBy('name')
                ->get(['id', 'name', 'slug']);
            return response()->json(['cities' => $cities], 200);
        } catch (\Exception $e) {
            \Log::error('citiesByState: ' . $e->getMessage());
