  ...
  public function choose_property_type()
{
    ...
    $user = User::select('id','name','email','image','phone','address','status')
    ...
    if ($homepage && $homepage->show_mobile_app == 'enable') {
        $app_visibility = true;
    }
    ...
    $property_content = (object) [
        ...
        // Buy (NEW PROPER IMPLEMENTATION)
        'buy_logo' => $setting->buy_logo,
        'buy_title' => $setting->buy_title,
        'buy_description' => $setting->buy_description,
        'buy_btn_text' => $setting->buy_btn_text,
    ];

    $breadcrumb = $setting->breadcrumb;

    return view('user.choose_property_type', compact(
        'mobile_app',
        'property_content',
        'breadcrumb',
        'user'
...
/*
|--------------------------------------------------------------------------
| VALID PURPOSE CHECK (UPDATED WITH BUY)
|--------------------------------------------------------------------------
*/
$validPurposes = ['rent', 'sale', 'buy'];

if (!in_array($request->purpose, $validPurposes)) {
    ...
    public function store(Request $request){
        ...
        $customMessages = [
            ...
            'country_id.nullable' => trans('user_validation.Country is nullable'),
